...
class Config:
    ...
    dropout: float = 0.15  # ViT dropout rate

    # ── Detection head ─────────────────────────────────────────────────────
    num_queries: int = 20  # Max QR codes per image
    num_queries_rows: int = 4 # Arrange queries in a 4x5 grid
    num_queries_cols: int = 5  # Arrange queries in a 4x5 grid
    # Each query predicts: [x1, y1, x2, y2] (normalized 0-1) + objectness

    # ── Loss weights ───────────────────────────────────────────────────────
    cost_class: float = 2.0  # Hungarian matching: class cost weight
    cost_bbox: float = 5.0  # Hungarian matching: L1 bbox cost weight
    cost_giou: float = 2.0  # Hungarian matching: GIoU cost weight
    loss_class: float = 3.0  # Training loss: classification weight
